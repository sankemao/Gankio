package sankemao.baselib.loadsir;
...
import sankemao.baselib.loadsir.core.TargetContext;
...
public class LoadSirUtil {
    public static TargetContext getTargetContext(Object target) {
        ...
        } else if (target instanceof View) {//给fragment设置的时候, 传入fragment的rootView.
            View view = (View) target;
            contentParent = (ViewGroup) (view.getParent());//如果是fragemnt则没有parent.
        ...
        View oldContent;
        if (target instanceof View) {//如果是fragment的情况
            oldContent = (View) target;//就是rootView.
            for (int i = 0; i < childCount; i++) {//childIndex = 0
