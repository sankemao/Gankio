...
import com.sankemao.quick.recyclerviewfixed.headfootview.LoadViewCreator;
...
public class LoadRefreshRecyclerView extends RefreshRecyclerView {
     ...
     * 刷新错误, 还原头部和尾部.
     */
    public void stopRefreshLoadByfail() {
        stopRefreshLoad(-1);
    }

    /**
     * 刷新成功, 处理头部和尾部.
     * 头部: limit > 0 需要考虑尾部, 总数据不足limit显示end, 大于等于limit还原尾部.
     * 尾部: limit > 0
     */
    ...
    public int getRealItemCount() {
        return getLayoutManager().getItemCount()
                - mWrapAdapter.getHeaderViewsCount()
